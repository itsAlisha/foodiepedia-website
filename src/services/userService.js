...
export const updateUser = async (userData) => {
  const { id, ...data } = userData; // Extract ID from userData
  const { data: responseData } = await axios.put(`/api/users/update`, {
    id,
    ...data,
  }); // Send the data in the body, not as part of the URL
  return responseData;
};
